...
export function coreProgress(unit:CoreUnit,entries:Entry[],now=Date.now()){
 const runs=independentRuns(unit.taskIds,entries).sort((a,b)=>a.time-b.time);
 ...
 const repeated=allSolved&&runs.some(e=>e.time>=completedAt+86400000&&runs.some(old=>old.task===e.task&&old.session!==e.session&&old.time<=completedAt));
 ...
 return {status,solved:solved.length,total:unit.taskIds.length,allSolved,repeated,due,reviewAt:completedAt?completedAt+86400000:0,nextTask:unit.taskIds.find(id=>!solved.includes(id))||unit.taskIds[2]} as const;
}
export function nextCoreTask(entries:Entry[],exclude='',now=Date.now()){
 ...
 if(due)return {id:coreProgress(due,entries,now).nextTask,reason:`Повторение «${due.title}»: восстановите решение спустя минимум сутки без подсказок.`};
